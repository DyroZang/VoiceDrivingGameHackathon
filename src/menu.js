...
export default function menu() {
  ...
  return (
    <>
      ...
      <div className="menuPage" style={{ backgroundImage: `url(${mapsImg})`, height: '90vh', backgroundPosition: 'center', backgroundSize: 'cover', backgroundRepeat: 'no-repeat' }}>
        <Row className="pt-5 m-0">
          <Col>
            <Button className="nameButton ms-auto d-block" onClick={() => setShow(true)}>{`${name ? name : 'Click to Enter Name'}`}</Button>
          </Col>
          <Col>
            <Button className="rightOtherButton my-3 me-auto d-block" onClick={() => navigate('/game')}>Play</Button>
          </Col>
        </Row>
        ...
        <Modal show={show} onHide={() => { if (name.length > 0) setShow(false) }}>
          ...
          <Modal.Body>
            ...
            <input className="form-control" value={name} placeholder="Name" onChange={handleName} />
          </Modal.Body>
          <Modal.Footer>
            <Button variant="primary" className="w-100" disabled={name.length == 0} onClick={() => setShow(false)}>Done</Button>
          </Modal.Footer>
        </Modal>
      </div>
    </>
